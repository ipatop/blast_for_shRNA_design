...
#Adapted for shRNA design by Reut Fluss, Ohad Bitton and Ines Patop
...
def main(args):
    ...
    NUM_OF_RESULTS =  args.res_num
    if args.output:
		outFile = open(args.output,'w')
    else:
	    outFile = open("blast_output_all",'w')
    outFile.write('name\tsequance\tidentities\tplus/minus\tgap\n')
    ...
    with open(args.input, 'r') as inputFile:
        while True:
                ...
                print ("blast for sequence: " , line[1][8:29])


                result_handle = qblast(program="blastn", database="refseq_rna", sequence=line[1][8:29], entrez_query= ORGANISM,hitlist_size=NUM_OF_RESULTS)
                ...
                for blast_record in blast_records:
                    for alignment in blast_record.alignments:
                        for hsp in alignment.hsps:
                            outFile.write(line[0] +'\t' + alignment.title +'\t' + str(hsp.identities)+'/'+str(hsp.align_length)+'\t'+str(hsp.strand)+'\t'+str(hsp.gaps)+'\n' )
...
if __name__ == '__main__':
    ...
    parser.add_argument("-input", "-in", help="Input peptides fasta file name", required=True)
    parser.add_argument("-output", "-out", help="Output file name", required=False)
    parser.add_argument("-res_num", "-n", help="number of results", required=False, default = 20)
    parser.add_argument("-ORGANISM", "-sp", help="species number as in NCBI", required=False, default = "txid7227[ORGN]")
